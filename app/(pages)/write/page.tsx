...
import { Info, Send, TrainFrontTunnel } from "lucide-react";
...
const Write = () => {
	...
	return (
		<div className="flex mt-20 gap-2 ml-16 mr-6 pt-2">
			<div className="w-8/12 p-10 rounded-md bg-white shadow-md shadow-slate-300 border border-slate-200">
				<div className="flex flex-col gap-3 items-start">
						...
						<div className="flex items-center gap-6">
							{image ? (
								<div className="relative mt-1">
									...
									<div
										onClick={() => setImage("")}
										className="h-8 w-8 flex items-center justify-center cursor-pointer bg-red-500 text-white hover:bg-red-600 rounded-full absolute -top-3 -right-3"
									>
										X
									</div>
								</div>
					...
					<InputTags
						...
						setShowTagsTooltip={setShowTagsTooltip}
						setTags={setTags}
					/>
				</div>
				<ReactQuill
					onFocus={() => setShowContentTooltip(true)}
					onBlur={() => setShowContentTooltip(false)}
				...
				<Button
					onClick={onSubmit}
					disabled={isLoading}
					className="bg-blue-700 disabled:opacity-60 text-white text-[16px] mt-14 flex gap-1 hover:bg-blue-800"
				>
					<Send size={15} />
					<span>Publish</span>
				</Button>
